...
execute as @a[tag=pairA,tag=alive] if score @s cupidPair = @p[tag=this] id run tag @s add currentTarget

# return pair_b book to cupid if other pair does not exist yet
execute unless entity @a[tag=currentTarget] as @p[tag=currentCupid,tag=!linked,tag=alive] run function ww:role/cupid/refund/pair_b
