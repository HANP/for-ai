...
execute if score %koumi_map system matches 0 run effect give @a night_vision infinite 0 true
...
execute if predicate ww:night if score %blood_moon system matches 1.. run function ww:role/wolf/bloody/blood_moon_effects
...
# Powder Particles
execute at @a[tag=powdered] run particle minecraft:happy_villager ~ ~0.2 ~ 0 0 0 0 1 force @a[tag=detective]
execute at @a[tag=powdered,tag=fool,tag=detective] run particle minecraft:happy_villager ~ ~0.2 ~ 0 0 0 0 1 force @a
