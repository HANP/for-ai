...
execute if score %bads_conquer system matches 1 run data modify storage ww:game DISPLAY.setting.ending set value '[{"text":" | 結局: "},{"text":"屠邊式","color":"green","bold":true}]'

execute if score %bads_massacre system matches 1 run data modify storage ww:game DISPLAY.setting.ending set value '[{"text":" | 結局: "},{"text":"屠城式","color":"green","bold":true}]'