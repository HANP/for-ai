...
# hentais check
execute if entity @a[tag=hentai,tag=alive] unless entity @a[tag=alive,tag=!harassed,tag=!hentai] run scoreboard players operation %ending system = %HENTAIS_WIN constant 
...
execute if predicate ww:started if score %ending system = %HENTAIS_WIN constant run function ww:ending/hentai
